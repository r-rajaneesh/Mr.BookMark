  ...
  DMBookmarks.forEach((bookmark: IDirectMessage) => {
    ...
    fetch(`https://discord.com/api/v10/channels/${OriginalMessage.channel}/messages/${OriginalMessageId}`, {
      ...
      .then(async (res) => {
        ...
        if (message?.code !== 10008) return;
        const DM: { id: string; user: string; channel: string }[] = DirectMessage;
        DM.forEach((dm) => {
          client.users.fetch(dm.user).then((user) => {
            user.dmChannel?.messages.fetch().then((messages) => {
              const msg = messages.get(dm.id) as Discord.Message;
  ...
  ChannelBookmarks.forEach((bookmark: IChannelMessage) => {
    ...
    fetch(`https://discord.com/api/v10/channels/${OriginalMessage.channel}/messages/${OriginalMessageId}`, {
      ...
      .then(async (res) => {
        const message = await res.json();
        if (message?.code !== 10008) return;
        const GuildChannel: { message: string; channel: string }[] = ChannelMessage;
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === "setchannel") {
    ...
    interaction.user
      ...
      .then(async (msg) => {
        const database: IDirectMessage = sql.prepare(`SELECT * FROM dmbookmark WHERE OriginalMessageId = (?)`).get(interaction.targetId) as IDirectMessage;

        if (!database) {
          sql.prepare(`INSERT INTO dmbookmark (OriginalMessageId, OriginalMessage, DirectMessage) VALUES (?, ?, ?)`).run(
    ...
    const GuildBookmark: IGuildSettings = sql.prepare(`SELECT * FROM guildbookmark WHERE GuildId = (?)`).get(interaction.guildId) as IGuildSettings;
    if (!GuildBookmark) {
      ...
      client.channels
        ...
        .then((channel) => {
          ...
          channel
            ...
            .then(async (msg) => {
              const database: IChannelMessage = sql
                .prepare(`SELECT * FROM channelbookmark WHERE OriginalMessageId = (?)`)
                .get(interaction.targetId) as IChannelMessage;

              if (!database) {
                sql.prepare(`INSERT INTO channelbookmark (OriginalMessageId, OriginalMessage, ChannelMessage) VALUES (?, ?, ?)`).run(
                  interaction.targetId,
                  JSON.stringify({
                    id: interaction.targetId,
                    channel: interaction.channelId,
    ...
    if (interaction.customId === "delete") {
      try {
        const database: IDirectMessage = sql
          .prepare(`SELECT * FROM dmbookmark WHERE OriginalMessageId = (?)`)
          .get(interaction?.message?.embeds[1]?.footer?.text) as IDirectMessage;
        await interaction.message.delete().catch((error) => {});
        if (!database) return;
        const updatedDMList = JSON.parse(database.DirectMessage).filter((val: { id: string; channel: string }) => val.id !== interaction.message.id);
        if (updatedDMList.length === 0) sql.prepare(`DELETE FROM dmbookmark WHERE OriginalMessageId = (?)`).run(interaction?.message?.embeds[1]?.footer?.text);
        else
          sql
...
client.on("messageReactionAdd", async (reaction, user) => {
  ...
  if (reaction.emoji.name == "🔖") {
    if (reaction.message.channel.type === Discord.ChannelType.DM || Discord.ChannelType.GroupDM) return;
    const message: Discord.Message = await reaction?.message.channel.messages?.fetch(reaction.message.id);
    if (!message) return;
    const attachments = message?.attachments;
    const msg = message?.content;
    const embeds = message?.embeds;
    ...
    user
      ...
      .then(async (msg) => {
        const database: IDirectMessage = sql.prepare(`SELECT * FROM dmbookmark WHERE OriginalMessageId = (?)`).get(reaction.message.id) as IDirectMessage;
        if (!database) {
          sql.prepare(`INSERT INTO dmbookmark (OriginalMessageId, OriginalMessage, DirectMessage) VALUES (?, ?, ?)`).run(
            reaction.message.id,
            JSON.stringify({
...
client
  .on("messageDelete", (message) => {
    ...
    if (client?.user?.id === message.author.id) {
      if (message.embeds.length !== 2) return;
      const msgid = message?.embeds?.[1]?.footer?.text;
      const entry: IChannelMessage = sql.prepare(`SELECT * FROM channelbookmark WHERE OriginalMessageId = (?);`).get(msgid) as IChannelMessage;
      if (!entry) return;
      const newEntries: { message: string; channel: string }[] = [] as { message: string; channel: string }[];

      const CMsg = JSON.parse(entry?.ChannelMessage);
      CMsg.forEach((msg: { message: string; channel: string }) => {
        if (msg.message !== message.id) newEntries.push(msg);
      });

      if (newEntries.length !== 0)
        sql.prepare(`UPDATE channelbookmark SET ChannelMessage = (?) WHERE OriginalMessageId = (?);`).run(JSON.stringify(newEntries), msgid);
      else sql.prepare(`DELETE FROM channelbookmark WHERE OriginalMessageId = (?)`).run(msgid);
      return;
    }
    if (message?.author?.bot) return;

    const DMBookmark: IDirectMessage = sql.prepare(`SELECT * FROM dmbookmark WHERE OriginalMessageId = (?)`).get(message.id) as IDirectMessage;

    const ChannelBookmark: IChannelMessage = sql.prepare(`SELECT * FROM channelbookmark WHERE OriginalMessageId = (?)`).get(message.id) as IChannelMessage;
    if (DMBookmark) {
      const DM: { id: string; user: string; channel: string }[] = JSON.parse(DMBookmark?.DirectMessage);
      DM.forEach((dm) => {
        client.users.fetch(dm.user).then((user) => {
          user.dmChannel?.messages.fetch();
          const msg = user.dmChannel?.messages.cache.get(dm.id) as Discord.Message;
          const originalEmbed = msg.embeds;
          const dataEmbed = new Discord.EmbedBuilder()
            .setAuthor({ name: `${originalEmbed[0].author?.name}`, iconURL: `${originalEmbed[0].author?.iconURL}` })
            .setDescription(`> Original message deleted by author`)
      ...
      const Channels: { message: string; channel: string }[] = JSON.parse(ChannelBookmark?.ChannelMessage);
...
type IDirectMessage = {
  ...
  OriginalMessage: string; // Details of the Original message
  DirectMessage: string; // Message IDs of the bookmarked message
};
type IChannelMessage = {
  OriginalMessageId: string; // Original Message ID
  OriginalMessage: string; // Details of the Original message
  ChannelMessage: string; // Message IDs of bookmarked message sent to guild channel
};
type IGuildSettings = { GuildId: string; ChannelId: string };
